          ...
          >
            <Select.Viewport>
              {items.map((item) => (
                <Select.Item
                  key={item.value}
                  className="flex cursor-default items-center p-custom10 text-size20 data-[highlighted]:bg-slate-100"
                  value={item.value}
                >
                  ...
                  <span className="text-slate-900 text-size20">{item.label}</span>
                </Select.Item>
              ))}
            </Select.Viewport>
